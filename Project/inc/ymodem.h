...
//#define PACKET_1K_SIZE          ((uint32_t)512)
...
#define NAK_TIMEOUT             ((uint32_t)0x400000)
#define DOWNLOAD_TIMEOUT        ((uint32_t)0x100000) //1000 0x100000) /* BUG BIG !!! One second retry delay */
#define MAX_ERRORS              ((uint32_t)5)

/* Exported functions ------------------------------------------------------- */
extern uint8_t *aPacketData;
COM_StatusTypeDef Ymodem_Receive( uint8_t *p_file_name,   uint8_t **p_buf, uint32_t * pfile_size);
COM_StatusTypeDef Ymodem_Transmit(uint8_t *p_file_name,   uint8_t *p_buf,  uint32_t  file_size);

static uint32_t Send_Byte (uint8_t c);

 /* Define the user application size */
#define USER_FLASH_SIZE               ((uint32_t)0x000004000)//3000) /* Small default template application */
...
// file comes from flash, otherwise from a long allocated buffer!
#define _USE_FLASH_SOURCE_
#ifdef  _USE_FLASH_SOURCE_
#define _HOST_RECV_SIZE_LIMIT_1KB_				(1024)
#endif
