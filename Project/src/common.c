/**
  ******************************************************************************
  * @file    STM32L1xx_IAP/src/common.c 
...
uint32_t SerialKeyPressedUsart3(uint8_t *key)
{
  ...
  if ( USART_GetFlagStatus(UartHandle, USART_FLAG_RXNE) != RESET)
  {
    *key = (uint8_t)UartHandle->DR;
    return 1;
  }
  else
  {
...
uint8_t GetKeyUsart3(void)
{
  ...
  while (1)
  {
    if (SerialKeyPressedUsart3((uint8_t*)&key)) break;
  }
  return key;
...
{
  while (USART_GetFlagStatus( UartHandle, USART_FLAG_TXE) == RESET) { };
